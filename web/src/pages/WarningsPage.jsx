    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10">
                ...
                <button
                    onClick={() => setHudSettings(prev => ({ ...prev, enableWarnings: !prev.enableWarnings }))}
                    className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${
                        hudSettings.enableWarnings ? 'bg-indigo-500' : 'bg-white/20'
                    }`}
                >
                    <motion.div 
                        className="absolute top-1 w-4 h-4 rounded-full bg-white shadow-md"
                        animate={{ x: hudSettings.enableWarnings ? 24 : 4 }}
            ...
            <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                <div className="flex justify-between mb-3">
                    ...
                    <span className="text-indigo-400 text-sm font-medium">{hudSettings.warningThreshold}%</span>
                </div>
                <input
                    type="range"
                    min={5}
                    max={50}
                    value={hudSettings.warningThreshold}
                    // @ts-ignore
                    onChange={(e) => setHudSettings(prev => ({ ...prev, warningThreshold: parseInt(e.target.value) }))}
            ...
            <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                ...
                <div className="flex gap-2">
                    {['#ef4444', '#f59e0b', '#eab308', '#ec4899', '#8b5cf6'].map((color) => (
                        <motion.button 
                            key={color} 
                            onClick={() => setHudSettings(prev => ({ ...prev, warningColor: color }))}
                            className={`w-8 h-8 rounded-full border-2 cursor-pointer ${
                                hudSettings.warningColor === color ? 'border-white' : 'border-transparent'
                            }`}
